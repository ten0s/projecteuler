...
main = do
    let res = sum [x | x <- [1..20161], isSumOfAbundants x == False] -- 9.422 sec
    print res

--
-- wget http://oeis.org/A048242/b048242.txt && gawk '{ s += $2 }END{ print s }' b048242.txt
--
isSumOfAbundants :: Int -> Bool
isSumOfAbundants m = isSumOfAbundants' m abundants
    where isSumOfAbundants' _ [] = False
          isSumOfAbundants' x (p:ps)
            | x < p = False
            | otherwise = if abundantsArray ! y then True else isSumOfAbundants' x ps
            where y = x - p

abundants = [x | x <- [1..20161], isAbundantNumber x]
abundantsArray = array (0, 20161) $ (0, False) : [(x, isAbundantNumber x) | x <- [1..20161]]

isPerfectNumber a = a == b
    where b = sum $ properDivisors a

isDeficientNumber a = a > b
    where b = sum $ properDivisors a

isAbundantNumber a = a < b
    where b = sum $ properDivisors a
